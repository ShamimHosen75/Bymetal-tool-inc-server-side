...
app.get('/', (req, res) => {
    res.send('Bymetal Tool Inc')
})

// mongodb url
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.tv4sw.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        const partsCollection = client.db("bymetal-tool-inc").collection("parts");
        const ordersCollection = client.db("bymetal-tool-inc").collection("orders");
        const paymentCollection = client.db("bymetal-tool-inc").collection("payment");
        const reveiwCollection = client.db("bymetal-tool-inc").collection("reveiws");
        const userCollection = client.db("bymetal-tool-inc").collection("user");
...
app.listen(port, () => {
    console.log(`Bymetal Server is Running ${port}`)
})